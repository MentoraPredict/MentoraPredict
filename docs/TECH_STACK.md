...
Este documento explica **por qué** se usa cada una de estas tecnologías en MentoraPredict, **qué problema resuelve** concretamente en el proyecto (no en abstracto) y **dónde** está implementada, con rutas de archivo verificables. Es un complemento a [ARCHITECTURE.md](ARCHITECTURE.md) (arquitectura de los microservicios backend) y a los [ADRs](adr/) (decisiones puntuales) — este documento cubre el stack de los cuatro clientes del proyecto (`apps/web`, `apps/landing`, `apps/mobile`, `apps/desktop`) y la capa de tiempo real.
...
**Dónde se usa**:
- Proyecto: `apps/desktop`, `electron-builder` como empaquetador, target `msi` únicamente (`win.target: "msi"` en `package.json`) — **no hay build de macOS ni Linux** pese a que en algún momento el copy de marketing lo insinuaba (corregido en `apps/web`/`apps/landing` para decir "Windows" únicamente).
...
|---|---|---|
| WebAssembly (Wasm) | No se usa | Sin ninguna referencia en el repo. No hay caso de uso identificado (no hay cómputo pesado del lado del cliente que lo justifique). |
| OpenVPN / OVPN | No se usa | Sin ninguna referencia en el repo. El acceso a infraestructura no pasa por una VPN propia. |
| Debounce | No implementado | Documentado como intención en `apps/web/docs/README.md`/`frontend-structure.md` (listan un `useDebounce.ts` como parte de la estructura de hooks planeada), pero ese archivo nunca se creó. Si se necesita (ej. un buscador que dispara una query por tecla), hay que escribirlo — no existe hoy. |
| Logging estructurado + tracing distribuido | Parcial | Hay un `CorrelationInterceptor` por microservicio que propaga `x-correlation-id`/`x-request-id` (setea headers y contexto de request, no hace logging estructurado por sí mismo) y un plugin `correlation-id` en Kong. Por separado, hay un stack de monitoreo real (`infra/monitoring/`: Grafana + Alertmanager, dashboards de salud de contenedores/infraestructura/servicios). Falta: una librería de logging estructurado (Winston/Pino) que efectivamente incluya esos IDs en cada línea de log, y tracing distribuido (OpenTelemetry/Jaeger) para seguir un request a través de varios servicios. |
