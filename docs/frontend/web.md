...
**Server/remote state** — TanStack Query (`queryClient` in `services/query/queryClient.ts`): `staleTime` 5 minutes, custom retry that only retries network/5xx errors (never 401/403/404/429) up to 2x, `refetchOnWindowFocus: false`. Persisted to `localStorage` via `PersistQueryClientProvider` in `App.tsx` under the key `mentorapredict-query-cache`, 7-day max age, with `createCourseMutation` mutation defaults registered synchronously at module load so paused mutations can resume after a cold reload.
