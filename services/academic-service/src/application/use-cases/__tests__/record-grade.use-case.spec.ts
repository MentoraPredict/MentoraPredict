...
describe("RecordGradeUseCase", () => {
  ...
  it("saves a valid grade for an enrolled student", async () => {
    ...
    const result = await useCase.execute(
      { studentId: "stud-1", evaluationId: "eval-1", value: 8.5 },
      "teacher-1",
    );
  ...
  it("throws NotFoundException if evaluation does not exist", async () => {
    ...
    await expect(
      useCase.execute(
        { studentId: "stud-1", evaluationId: "bad-id", value: 7 },
        "teacher-1",
      ),
  ...
  it("throws BadRequestException if student is not enrolled", async () => {
    ...
    await expect(
      useCase.execute(
        { studentId: "stud-1", evaluationId: "eval-1", value: 7 },
        "teacher-1",
      ),
  ...
  it("throws ConflictException if grade already exists", async () => {
    ...
    await expect(
      useCase.execute(
        { studentId: "stud-1", evaluationId: "eval-1", value: 8 },
        "teacher-1",
      ),
