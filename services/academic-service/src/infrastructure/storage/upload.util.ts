...
// (kong-template.yml: academic-uploads, strip_path: false, sin plugin jwt).
export const UPLOADS_URL_PREFIX = '/api/v1/academic/uploads';
...
// Fase 10b — temario: un Topic puede llevar una imagen O un documento.
...
// Multer's own `limits.fileSize` must accept the larger of the two ceilings
// up front (real documents shouldn't be truncated mid-stream); the precise
// per-type ceiling is enforced afterward by validateTopicFile below.
export const MAX_TOPIC_FILE_BYTES = MAX_DOCUMENT_BYTES;
...
// the client's original filename is never used for the path, only kept as
// metadata by the caller if it wants to display it.
...
// Accepts an image OR a document (jpg/jpeg/png at 2MB, pdf/docx/pptx/xlsx at
// 10MB) — used only by Topic file uploads. Subject image/avatar keep using
// validateImage above, untouched.
export function validateTopicFile(file: MulterMemoryFile | undefined): void {
  if (!file?.buffer) throw new BadRequestException('Se requiere un archivo');

  const isImage = !!IMAGE_MIME_EXT[file.mimetype];
  const isDocument = !!DOCUMENT_MIME_EXT[file.mimetype];
  if (!isImage && !isDocument) {
    throw new UnsupportedMediaTypeException(
      `Tipo de archivo no permitido: ${file.mimetype}. Se aceptan jpg, jpeg, png, pdf, docx, pptx o xlsx.`,
    );
  }
  ...
  if (file.size > maxBytes) {
    throw new PayloadTooLargeException(
      `Archivo demasiado grande. Máximo ${isImage ? '2MB para imágenes' : '10MB para documentos'}.`,
    );
  }
}

export function saveTopicFileToDisk(subdir: string, file: MulterMemoryFile): { publicUrl: string; relativePath: string } {
  const ext = IMAGE_MIME_EXT[file.mimetype] ?? DOCUMENT_MIME_EXT[file.mimetype];
  const storedFileName = `${randomUUID()}.${ext}`;
  const dir = path.join(UPLOADS_ROOT, subdir);
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, storedFileName), file.buffer);
