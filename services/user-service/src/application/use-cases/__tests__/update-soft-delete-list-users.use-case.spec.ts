...
import { AuthProvider, UserProfileEntity } from '../../../domain/entities/user-profile.entity';

const makeProfile = (id = 'uid-1') =>
  new UserProfileEntity(
    id, null, null, '1234567890', AuthProvider.LOCAL, 'STUDENT', 'ACTIVE',
...
describe('UpdateUserUseCase', () => {
  ...
  it('throws NotFoundException when user does not exist', async () => {
    ...
    const useCase = new UpdateUserUseCase(repo);
    await expect(useCase.execute('missing', { bio: 'x' })).rejects.toThrow(NotFoundException);
...
describe('ListUsersUseCase', () => {
  ...
  it('passes role and status filters to the repository', async () => {
    ...
    expect(repo.findAll).toHaveBeenCalledWith({ role: 'TEACHER', status: 'ACTIVE' });
  });
