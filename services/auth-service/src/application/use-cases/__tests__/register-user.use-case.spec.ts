...
describe('RegisterUserUseCase', () => {
  ...
  it('throws ConflictException when email already exists', async () => {
    const existing = new UserEntity(
      'uuid-1', 'victor@uce.edu.ec', 'hash',
      UserRole.STUDENT, true, false, new Date(), new Date(),
    );
    repo.findByEmail.mockResolvedValue(existing);

    await expect(
      useCase.execute({ firstName: 'V', lastName: 'C', email: 'victor@uce.edu.ec', password: 'SecurePass123!' }),
  ...
  it('throws error if email is invalid', async () => {
    await expect(
      useCase.execute({ firstName: 'V', lastName: 'C', email: 'not-an-email', password: 'SecurePass123!' }),
    ).rejects.toThrow();
  });
  ...
  it('throws error if password is too short', async () => {
    repo.findByEmail.mockResolvedValue(null);
    await expect(
      useCase.execute({ firstName: 'V', lastName: 'C', email: 'v@uce.edu.ec', password: 'short' }),
    ).rejects.toThrow();
  });
});
