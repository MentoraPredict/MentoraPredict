...
describe("CalculateTrendUseCase", () => {
  it("throws when fewer than 3 periods provided", async () => {
    const useCase = new CalculateTrendUseCase(
      mockMetricsRepo(),
      mockDatasetRepo(),
    );
    await expect(useCase.execute("s1", ["p1", "p2"])).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("classifies ascending trend", async () => {
    ...
    metricsRepo.findByStudentAndPeriod
      .mockResolvedValueOnce(
        new StudentMetricsEntity("m1", "s1", "p1", {}, 5, new Date(), 1),
      )
      .mockResolvedValueOnce(
        new StudentMetricsEntity("m2", "s1", "p2", {}, 6, new Date(), 1),
      )
      .mockResolvedValueOnce(
        new StudentMetricsEntity("m3", "s1", "p3", {}, 9, new Date(), 1),
      );
